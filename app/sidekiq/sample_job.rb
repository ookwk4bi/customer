...
class SampleJob
  include Sidekiq::Job
  sidekiq_options queue: :aimitsusaas_scraping
  ...
  def perform(imitsu_id)
    # 引数subject_idのID情報からすべてのカラム情報を取得。
    imitsu = Imitsu.find_by(id: imitsu_id)
    filedir  = "#{Rails.root}/tmp" # tmp配下に保存されて欲しいのでtmpのディレクトリパスを変数に入れる
    filename = "#{imitsu.imitsu_name}_#{Time.zone.now.strftime('%Y%m%d')}.csv"
    ...
    session.visit imitsu.imitsu_url
    sleep(2)
    # html情報をnokogiriで処理
    ...
    loop do
      ...
      list_paths.each.with_index(1) do |list_path, i|
        ...
        # xpathを元に要素のテキストを取得
        name = doc.xpath("//div[contains(text(),'会社名')]/following-sibling::div").text
        address = doc.xpath("//div[contains(text(),'本社所在地')]/following-sibling::div").text.squish
        url = doc.xpath("//span[text()='公式サイトへ']/../@href").text
        puts name #デバック用
        # ハッシュを作成する
      ...
      if pages.present?
        session.find("//li[@class='current']/following-sibling::li").click
        sleep(1)
      else
        break
      end
