...
class RekaizenJob
  include Sidekiq::Job
  sidekiq_options queue: :rekaizen_scraping
  ...
  def perform(rekaizen_id)
    # 引数subject_idのID情報からすべてのカラム情報を取得。
    rekaizen = Rekaizen.find_by(id: rekaizen_id)
    filedir  = "#{Rails.root}/tmp" # tmp配下に保存されて欲しいのでtmpのディレクトリパスを変数に入れる
    filename = "#{rekaizen.rekaizen_name}_#{Time.zone.now.strftime('%Y%m%d')}.csv"
    ...
    session.visit rekaizen.rekaizen_url
    sleep(2)
    session.execute_script "window.scrollBy(0,10000)"
    sleep(2)
    # html情報をnokogiriで処理
    doc = Nokogiri::HTML.parse(session.html)
    koukoku = doc.xpath("//div[@class='howtomodal visible' and @style='']")
    if koukoku.present? # Nokogiri::HTML.parse(session.html).xpath("//span[@class='close-btn']").present?
      # あった場合要素をクリック.
      session.find("//input[@name='next_no_show']").click
      session.find("//button[@class='btn close_btn']").click
    end
    # 結果格納用配列→CSV保存の処理のため。
    companies = Array.new
    # ループ処理
    ...
    rekaizen.csv_file.attach(io: File.open(filepath), filename: filename, content_type: "text/csv")
    # 作成したCSVファイルを削除。
